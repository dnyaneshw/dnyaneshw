...
namespace Insurance.UserControl
{
    public partial class ucCreditCardInformation : System.Web.UI.UserControl
    {
        ...
        public bool Is4Valid()
        {
            ...
            else if (txtPZip.Text == "")
            {
                lbl2.Visible = true;
                RequiredFieldValidator19.Style.Value = "display:none;";
                lbl2.Text = "Please Enter Postal Code.";
                return false;
            }
            if ((txtPZip.Text.Length != 7) || (!Regex.IsMatch(txtPZip.Text, @"[ABCEGHJKLMNPRSTVXYabceghjklmnprstvxy][0-9][ABCEGHJKLMNPRSTVWXYZabceghjklmnprstvwxyz] ?[0-9][ABCEGHJKLMNPRSTVWXYZabceghjklmnprstvwxyz][0-9]$")))            
            {
                lbl2.Visible = true;
                RequiredFieldValidator19.Style.Value = "display:none;";
                lbl2.Text = "Please Enter Valid Postal Code Format.";
                return false;
            }
            return true;
        }
