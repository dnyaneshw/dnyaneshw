...
function isZipCode(str) {
    var objRegExp = /^[ABCEGHJKLMNPRSTVXYabceghjklmnprstvxy][0-9][ABCEGHJKLMNPRSTVWXYZabceghjklmnprstvwxyz] ?[0-9][ABCEGHJKLMNPRSTVWXYZabceghjklmnprstvwxyz][0-9]$/;
    return (objRegExp.test(str)) ? true : false;
}
...
function Validate( objPersonalID, objFirstName, objLastName, objAddress, objEmailID, objphoneNo, objCity, objState, objZipCode, objCompnyAffil, objAgree) {
    ...
    if (State.value == "") {
        alert('Please Enter Province or Territory.');
        State.focus();
        return false;
    }
